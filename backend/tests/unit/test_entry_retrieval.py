...
from app.services.entry_retrieval import rank_entries, select_entries
...
def _request(**updates) -> EntryRetrieveRequest:
...
def test_keyword_match_and_stable_id_tie_break() -> None:
    ...
    tied = rank_entries([_entry("b"), _entry("a")], _request())
...
def test_budget_selects_whole_entries_and_skips_oversize_candidates() -> None:
    ...
    selected, rejected = select_entries(rank_entries([oversized, small], request), request)
...
def test_exemplar_boost_only_applies_to_voice_related_tasks() -> None:
    ...
    general = rank_entries([exemplar], _request())
    voice = rank_entries(
        [exemplar], _request(task_kind=EntryRetrievalTaskKind.VOICE)
    )
...
def test_cast_and_location_are_identity_hints_not_scope_expansion() -> None:
    ...
    ranked = rank_entries(
        ...
        _request(cast=["character-1"], location="world-1"),
    )
