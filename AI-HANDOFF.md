...
PR #31's independent reviews found five preparation correctness issues plus one tied-order compatibility edge, now fixed on the same Draft branch. Character and Lore loading preserve the legacy DB-query-supplied sequence when the domain has no canonical equal-key order; AOS-1 adds no UUID/name/id tie-break and snapshots the supplied sequence immutably. A frozen provider-prompt fixture covers both distinct and identical timestamps with Character and same-priority Lore inserted Z then A. Production and the P1-8 Novel shadow share one owner/deleted-safe Character/World read helper. The pure boundary does not reclassify chronology, but rejects legacy or Entry summary evidence that cannot prove a same-Work, non-target, strictly prior Chapter. Every public sequence/mapping is defensively snapshotted; typed preparation fields are the single canonical authority, sections/evidence are deterministic derived views, and `PreparedPromptBlock` is only a whitelisted PromptEngine compatibility value. Entry metadata and nested chronology metadata no longer pass through arbitrary keys, and omission evidence uses the Entry type's actual semantic section.
