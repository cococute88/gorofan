...
**P1-8 design and implementation:** [Legacy Context ↔ Entry Equivalence Bridge](legacy-entry-equivalence-design.md) fixes the read-only projection, independent coverage/runtime states with coexisting diagnostics, runtime-selection trace, and later lore-scanner cutover gate. PR #28 merged the diagnostic implementation; it authorizes no backfill, migration, read cutover, or feature-flag change.

**Post-P1-8 chronology contract:** [Story Summary Chronology Contract](story-summary-chronology.md) defines the required target-Chapter anchor, strict-prior eligibility, fail-safe future/current/unknown exclusion, ascending story order, lifecycle/duplicate handling, and budget boundary for Chapter-level `story.summary` context. It is a documentation-only correctness gate; implementation follows only after independent review and merge, before shared Generation Preparation.
