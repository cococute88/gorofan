...
- **Conforms to:** RFC-001, RFC-002, RFC-003, RFC-004, RFC-005, RFC-006, RFC-007, RFC-008; ADR-013, ADR-009, ADR-016, ADR-005, ADR-008, ADR-012, ADR-001
- **Supersedes:** nothing
...
> **Reading order.** RFC-001 is the system-level reference; RFC-002 the Entry Store; RFC-003 the Analyst; RFC-004 the Writer; RFC-005 the Story Bible; RFC-006 the Relationship model; RFC-007 Character DNA; RFC-008 Retrieval & Context Assembly. Read them first. This RFC defines the **Prompt System** — the layer responsible for turning *assembled context* into a *provider-ready prompt*, and for defining how prompts are **composed, versioned, and evolved** over time. It explains *why it exists* and *what it owns and does not own*. It does **not** define prompt contents, templates, wording, block structures, models, or provider APIs — each is named and deferred.
>
> **Source of truth.** The RFC documents take precedence over this one, in order (RFC-001, then RFC-002…RFC-008); behind them the ADR set (`docs/architecture/adr/`) is authoritative, and the two reviews (`docs/design-review-ai-author-os.md`, `docs/architecture-final-minimal.md`) supply rationale only. Where anything here appears to conflict with an RFC or an ADR, **those win** and this document is in error.
>
> **Relationship to RFC-008.** RFC-008 defines the runtime *mechanism* — the deterministic, budgeted, provider-neutral **assembly pipeline** that turns retrieved knowledge and inputs into context. This RFC defines the **prompt bodies as versioned architectural assets**, the **composition model** that combines those bodies with assembled context, the **provider-neutral boundary**, and the **Bench-gated evolution workflow**. The two are complementary: RFC-008 owns *how context is assembled*; RFC-009 owns *what prompts are, how they are versioned, and how they evolve*.
>
...
Every generation the product performs is, at the last step, a **prompt** — and the quality of that prompt is the quality ceiling of the output. Assembled context (RFC-008) is the *material*; a prompt is what that material becomes when combined with the authored, versioned instruction that tells the model what to do with it. The **Prompt System** is the layer that owns that transformation and, more importantly, owns **how prompts are composed, versioned, and evolved over the life of the product**.
...
- **Prompt budgeting (budget-aware composition).** Composing within the token budget so the result always fits, with the important material preserved and only the truncatable material trimmed (ADR-009 §2–§3). The budget *guarantee* is realized by the assembly mechanism (RFC-008 §3, §6); the Prompt System owns that composition is *always* budget-aware and never produces an unbudgeted prompt (ADR-009 §2).
...
- **Retrieval.** The Prompt System does not select which knowledge enters a prompt. Selection is **Retrieval**, the Store's one capability (RFC-002 §8; RFC-008 §4). The Prompt System composes with whatever context assembly hands it; it does not choose that context.
- **Knowledge extraction.** The Prompt System does not turn text into knowledge. Extraction is the **Analyst's** job (RFC-003 §3). The Analyst's facets are prompt bodies the Prompt System versions — but *running* extraction, and owning its output, belongs to the Analyst.
...
- **Final prompt — deterministic, budgeted, provider-neutral.** Composing a template with its blocks yields the final prompt: fitted to budget by priority, reproducible given its inputs, and emitted in a provider-neutral form the adapter renders provider-ready (RFC-008 §6–§7; ADR-009 §2, §4). The composition is a function of its inputs — the same template and the same blocks compose the same final prompt (RFC-008 §7).
...
The Prompt System **consumes the output of retrieval and assembly**; it does not perform them (RFC-008 §3–§4).

- **Retrieval selects; the Prompt System composes.** Retrieval — the Store's one capability — chooses the minimum necessary knowledge for the task (RFC-008 §4). Context assembly turns that selection (plus non-knowledge inputs) into blocks (RFC-008 §3). The Prompt System composes those blocks with the authored template into the final prompt (§5). Three clean handoffs: *select → assemble → compose*, each owned by a different responsibility.
- **The Prompt System never reaches into the Store.** It composes with whatever blocks assembly provides; it does not query the Store, hand-select knowledge, or re-rank — selection stays wholly in retrieval, keeping the boundary clean (RFC-008 §3, §8; ADR-009 §v2-note). If a prompt is missing knowledge, the fix is in retrieval, not in the prompt body inventing facts.
- **Composition inherits assembly's determinism and budget guarantee.** Because the Prompt System composes through the same deterministic, budget-enforcing assembler (RFC-008 §6–§7), its final prompts are reproducible and always within budget — the properties the Bench and debugging depend on (§9; RFC-008 §7). The Prompt System relies on, and does not duplicate, RFC-008's mechanism. **This RFC does not redefine retrieval or assembly — RFC-008 does.**
...
- **Measurability depends on deterministic composition.** The Bench can only attribute a quality change to a prompt change if composition is deterministic given fixed inputs (RFC-008 §7). The Prompt System's deterministic, file-based, single-source composition is therefore the precondition that makes prompt evaluation meaningful — versioning and measurability reinforce each other (§6, §7; RFC-008 §7). **This RFC does not redefine the Bench — the corresponding RFC does.**
...
- **New context sources are new block kinds, not new machinery.** When a new kind of knowledge should enter prompts, it becomes a new block with a priority in the existing composition model — absorbed without new assembly machinery (RFC-008 §11; ADR-009 §5). The composition model was built to be the natural insertion point for every future context source.
...
**Not in the final prompt — by invariant — but the risk is real upstream.** The composition budget guarantees the *assembled* prompt never overflows the context window, dropping only truncatable blocks and never the protected ones (RFC-008 §12.2; ADR-009 §2–§3). The honest residual risks are about *composition under pressure*, not overflow:
...
**On the Bench, against frozen golden scenarios — never by intuition** (§10; ADR-012). Because prompts are the product's highest-leverage, most-changed text, their quality is evaluated by measurement: every change A/B-compared on golden scenes using the built-in checks as metrics, with deterministic composition ensuring the result is attributable to the change (RFC-008 §7; ADR-012). Priority tuning, if it becomes a recurring lever, is itself surfaced to the Bench so it is measured rather than guessed (ADR-009 §6). *The Bench is Defined in the corresponding RFC.*

### 12.4 When should prompt structure change?

**Rarely, and only when composition itself — not a prompt's content — is the limit.** The default is that behavior evolves by editing prompt *bodies* within the fixed composition model (§11); the *structure* (the block/priority model, the neutral contract, the pipeline) is deliberately the still part (RFC-008 §6; ADR-009 §2). Structure changes only on a real, visible trigger:
...
- **The composition/budget mechanism changes** only if measured over/under-budget behavior or a tokenizer limit demonstrably hurts — a tuning of the mechanism, not a redesign (ADR-009 §6; RFC-008 §12).
...
- **Prompt blocks** — the `PromptBlock` structure, kinds, priority scheme, truncatable semantics. *Defined in the corresponding RFC (the assembly mechanism, RFC-008 territory).*
...
| **The Retrieval & Context Assembly RFC (RFC-008)** | The assembled context blocks the Prompt System composes into final prompts (mutual: it supplies the mechanism this system relies on). |
...
|---|---|
| §1 Purpose | RFC-001 §2.4, §8.3; ADR-013 §2; ADR-009 §1; RFC-008 §3 |
| §2 Why the Prompt System Exists | RFC-001 §2.4, §8.3; ADR-009 §1–§2, §4-A, §5; ADR-013 §5 |
| §3 Responsibilities | ADR-009 §2–§4; ADR-013 §1–§3; ADR-005 §2; ADR-008 §2; ADR-016 §2; RFC-008 §3, §6 |
| §4 Does NOT Own | RFC-001 §4; RFC-002 §6.1, §8, §3.4; RFC-003 §3; RFC-004 §3; RFC-005 §5; RFC-008 §4; ADR-013 §3; ADR-016 §2 |
| §5 Prompt Composition Philosophy | ADR-009 §2–§3; ADR-013 §1, §3; ADR-005 §2; ADR-008 §2; RFC-008 §6–§7 |
...
| §8 Relationship with Retrieval | RFC-008 §3–§4, §6–§8, §11; ADR-009 §4, §v2-note; ADR-016 §2 |
| §9 Relationship with Writer | RFC-004 §3–§4, §9; ADR-005 §2; ADR-013 §1; RFC-003 §8; RFC-001 §2.4 |
| §10 Relationship with Bench | RFC-001 §2.9; ADR-012; ADR-013 §1, §4; `architecture-final-minimal.md` §7; RFC-008 §7 |
| §11 Evolution Strategy | RFC-001 §7, §7.2, §7.4; ADR-005 §2; ADR-008 §2; ADR-013 §1, §3, §6; ADR-009 §4-C, §5, §6; ADR-016 §6; RFC-008 §11 |
| §12 Architectural Risks | ADR-009 §2–§3, §5–§6; ADR-013 §2, §5; ADR-012; RFC-008 §7, §12; RFC-001 §7.4 |
| §13 Out of Scope | RFC-001 §9 (RFC boundary conventions) |
| §14 Dependencies | RFC-001 §10; RFC-008 §14 |
