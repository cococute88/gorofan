...
`legacy_not_runtime_visible` is a diagnostic code/visibility value, not a coverage result. Other codes may coexist, including `duplicate_legacy_payload`, `ignored_disabled_lorebook`, `legacy_disabled_entry`, `legacy_keyword_miss`, `legacy_resolved_payload_empty`, `legacy_equal_priority_order_unspecified`, `entry_status_excluded`, `entry_orphan_excluded`, `entry_limit_rejected`, `entry_retrieval_budget_rejected`, `entry_rendered_budget_rejected`, `final_prompt_budget_drop`, `future_story_position_selected`, `future_story_position_not_selected`, `unknown_story_position`, `unknown_story_position_selected`, `unknown_story_position_not_selected`, `database_timestamp_recency`, `memory_evaluation_time_required`, `regenerate_assistant_timestamp_tie`, and `regenerate_user_timestamp_tie`.
...
Final-budget survival is attributed at source level. For aggregate Character, World, and Novel system blocks, the diagnostic carries the exact resolved character offsets, explicit resolved-payload presence, and full normalized payload digest for each source field through the shadow assembly. A non-empty stored projection remains part of coverage even when variable resolution makes its runtime payload empty; at runtime that zero-length `(start, start)` span is `legacy_resolved_payload_empty`, never survival evidence merely because the empty-slice digest matches. The diagnostic never infers one source's survival from `payload in aggregate_block`, from a bounded preview, or from another source containing the same text. Identical and partially overlapping values therefore retain distinct source identity and occurrence count. One-source Lore and Chapter blocks require full normalized block equality after final budgeting, so a truncated prefix is not treated as survival. This trace is diagnostic-only and does not add markers to, or otherwise change, provider-visible prompt text.

Applicable runtime order follows provider-visible rendering evidence: prompt block position first, then the aggregate source span's resolved start/end offsets. `source_key` remains identity/tie evidence only and cannot replace an aggregate's inner rendered position. Equal-priority Lore with production-unspecified order remains `unsupported_legacy_semantics` and is not folded into an artificial stable sequence.
...
- legacy projection resolved to an empty provider-visible payload;
...
For Novel, the report resolves both a summary subject Chapter index and `created_at_chapter_id` index when present. Chronology classification is independent of coverage matching and final runtime selection: every eligible exact-matched or entry-only `story.summary` is classified from its position, then receives selected/not-selected terminology from final PromptEngine survival rather than retrieval candidacy. A runtime-selected row tied to a Chapter at or after the target Chapter is `future_story_position_selected`; a row rejected by retrieval, Context Assembly, or final PromptEngine budgeting instead receives `future_story_position_not_selected` while retaining that stage's exclusion code. Missing/unresolvable position retains the general `unknown_story_position` evidence and is additionally distinguished as `unknown_story_position_selected` or `unknown_story_position_not_selected`. The current ranker's use of `updated_at`/`created_at` is reported as `database_timestamp_recency` and is never normalized into story chronology by the bridge.
